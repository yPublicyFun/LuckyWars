...
class LuckyWars extends PluginBase implements Listener {
    ...
    public $prefix = TE::GRAY . "[" . TE::BLUE . TE::RESET . "Lucky" . TE::BLUE . "Wars" . TE::RESET . TE::GRAY . "]";
	public $mode = 0;
	public $arenas = array();
	public $currentLevel = "";
        public $op = array();
	...
	{
		  $this->getServer()->getLogger()->info(TE::DARK_AQUA . "LuckyWars wurde aktiviert von xEchtesPhantom");
						...
						if($args[0]=="addarena")
						{
							if(!empty($args[1]))
							{
								if(file_exists($this->getServer()->getDataPath() . "/worlds/" . $args[1]))
								{
									...
									$this->mode = 1;
									$player->sendMessage($this->prefix . "Setzte nun die Spawns von allen Spielern!");
								...
								{
									$player->sendMessage($this->prefix . "Fehler,benötige eine Welt!");
								}
							}
							else
							{
								$player->sendMessage($this->prefix . "Fehler,benötige richtigen Commands!");
							}
						}
						else
						{
							$player->sendMessage($this->prefix . "Falscher Command!");
						}
					}
					else
					{
					 $player->sendMessage($this->prefix . "Commands von LuckyWars");
                                         $player->sendMessage($this->prefix . "/lw addarena [welt]: Erstellt eine Arena!");
                                         $player->sendMessage($this->prefix . "/lwstart: Startet eine Runde");
					}
				}
			return true;
                            ...
                            if($player->isOp())
				{
                                if(!empty($args[0]))
					{
                                        ...
                                        $config->save();
                                        $player->sendMessage($this->prefix . "§aStartet in 10...");
                                        }
                                        }
                                        else
                                        {
	...
	public function onInteract(PlayerInteractEvent $event)
	{
		...
		if($tile instanceof Sign) 
		{
			...
			{
				$tile->setText(TE::GREEN . "[Beitreten]",TE::YELLOW  . "0 / 12","§f" . $this->currentLevel,$this->prefix);
				$this->refreshArenas();
				$this->currentLevel = "";
				$this->mode = 0;
				...
				if($text[3] == $this->prefix)
				{
					if($text[0]==TE::GREEN . "[Beitreten]")
					{
						...
						$level = $this->getServer()->getLevelByName($namemap);
                                                ...
                                                $player->sendMessage($this->prefix."Es gibt keine gesetzten Spawns!");
                                                goto sinslots;
                                                with:
                                                $slots->save();
                                                $player->sendMessage($this->prefix . "§7Du bist der LuckyWars Arena beigetreten");
                                                foreach($level->getPlayers() as $playersinarena)
                                                {
                                                $playersinarena->sendMessage($player->getName() ." §7hat das Spiel betreten");
                                                }
					...
					{
						$player->sendMessage($this->prefix . "Du kannst dieser Runde nicht beitreten!");
					}
				}
			}
		}
		elseif(in_array($player->getName(), $this->op)&&$this->mode>=1&&$this->mode<=11)
		{
			...
			$player->sendMessage($this->prefix . "Spawn " . $this->mode . " wurde erfolgreich registriert!");
			$this->mode++;
			$config->save();
		}
		elseif(in_array($player->getName(), $this->op)&&$this->mode==12)
		{
			...
			$player->sendMessage($this->prefix . "Tippe nun ein Schild an um die Runde zu registrieren");
			$spawn = $this->getServer()->getDefaultLevel()->getSafeSpawn();
			$this->getServer()->getDefaultLevel()->loadChunk($spawn->getFloorX(), $spawn->getFloorZ());
			$player->teleport($spawn,0,0);
			$config->save();
		...
		foreach($tiles as $t) {
			if($t instanceof Sign) {	
				...
				if($text[3]==$this->prefix)
				{
					...
					$ingame = TE::GREEN . "[Beitreten]";
					$config = new Config($this->plugin->getDataFolder() . "/config.yml", Config::YAML);
					if($config->get($namemap . "PlayTime")!=780)
					{
						$ingame = TE::RED . "[InGame]";
					}
					elseif($aop>=12)
					{
						$ingame = TE::DARK_RED . "[Voll]";
					}
					...
					else
					{
						...
						if($config->get($arena . "inicio")==1)
						{
							if($timeToStart>0)
							{
								...
								{
									$pl->sendPopup(TE::GREEN."Die Runde starte in ".TE::GREEN . $timeToStart.TE::RESET);
                                                                        if($timeToStart<=5)
                                                                        {
							...
							else
							{
								...
								if($aop==1)
								{
									foreach($playersArena as $pl)
									{
                                                                            $this->plugin->getServer()->broadcastMessage($this->prefix.$pl->getName().TE::GREEN." ist der Gewinner in der Map".TE::AQUA.$arena);
                                                                            $pl->getInventory()->clearAll();
                                                                            $pl->removeAllEffects();
                                                                ...
                                                                if(($aop>=2))
                                                                {
                                                                    foreach($playersArena as $pl)
                                                                        {
                                                                        $pl->sendPopup(TE::BOLD.TE::GOLD.$aop." ".TE::AQUA." Verbleibende Spieler".TE::RESET);
                                                                        }
                                                                }
								$time--;
								if($time == 779)
								{
									...
									foreach($playersArena as $pl)
									{
										$pl->sendMessage(TE::GRAY.">--------------------------------");
                                                                                $pl->sendMessage(TE::DARK_GRAY.">".TE::RED."Start: ".TE::GREEN."Die Runde wurde gestartet!");
                                                                                $pl->sendMessage(TE::DARK_GRAY.">".TE::RED."Map: " .TE::GREEN. $arena);
                                                                                $pl->sendMessage(TE::GRAY.">--------------------------------");
									}
								}
                                                                if($time == 550)
								{
									foreach($playersArena as $pl)
									{
                                                                                $pl->sendMessage(TE::DARK_GRAY.">".TE::AQUA."Programmiert von xEchtesPhantom");
                                                                                $pl->sendMessage(TE::DARK_GRAY.">".TE::AQUA."".TE::GREEN."");
									}
								}
								if($time<300)
								{
									...
									if(is_int($minutes) && $minutes>0)
									{
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix .TE::YELLOW. $minutes . " " .TE::GREEN. " Minuten Verbleiben");
										}
									}
									else if($time == 30 || $time == 15 || $time == 10 || $time ==5 || $time ==4 || $time ==3 || $time ==2 || $time ==1)
									{
										foreach($playersArena as $pl)
										{
											$pl->sendMessage($this->prefix .TE::YELLOW. $time . " " .TE::GREEN. " sekunden verbleiben");
										}
									}
									if($time <= 0)
									{
                                                                            $this->plugin->getServer()->broadcastMessage($this->prefix .TE::GREEN." Es gibt keine Gewinner in der Map ".TE::AQUA.$arena);
										foreach($playersArena as $pl)
										{
											$pl->teleport($this->plugin->getServer()->getDefaultLevel()->getSafeSpawn(),0,0);
											$pl->getInventory()->clearAll();
						...
						else
						{
                                                    foreach($playersArena as $pl)
                                                    {
                                                            $pl->sendPopup(TE::GRAY . "Benötige Mehr Spieler..." .TE::RESET);
                                                    }
                                                    $config->set($arena . "PlayTime", 780);
                                                    $config->set($arena . "StartTime", 30);
